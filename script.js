function abrirmenu(){
  document.getElementById("menu").style.marginLeft = "0";
}

function cerrarmenu(){
  document.getElementById("menu").style.marginLeft = "-100%"
}

function cambiaAk47(){

  document.getElementById("scene").src = "3dmodels/ak47.glb";
  document.getElementById("menu").style.marginLeft = "-100%";
}

function cambiaSniper() {

  document.getElementById("scene").src = "3dmodels/sniper.glb";
  document.getElementById("menu").style.marginLeft = "-100%";
}

function cambiaTierra() {

  document.getElementById("scene").src = "3dmodels/tierra.glb";
  document.getElementById("menu").style.marginLeft = "-100%";
}

function cambiaTelefono() {

  document.getElementById("scene").src = "3dmodels/telefono.glb";
  document.getElementById("menu").style.marginLeft = "-100%";
}

function cambiaDaga() {

  document.getElementById("scene").src = "3dmodels/daga.glb";
  document.getElementById("menu").style.marginLeft = "-100%";
}

function cambiaSwitch() {

  document.getElementById("scene").src = "3dmodels/Switch.glb";
  document.getElementById("menu").style.marginLeft = "-100%";
}

function cambiaTrex() {

  document.getElementById("scene").src = "3dmodels/trex.glb";
  document.getElementById("menu").style.marginLeft = "-100%";
}

function cambiaCasa() {

  document.getElementById("scene").src = "3dmodels/casa.glb";
  document.getElementById("menu").style.marginLeft = "-100%";
}

function cambiaDragon() {

  document.getElementById("scene").src = "3dmodels/dragon.glb";
  document.getElementById("menu").style.marginLeft = "-100%";
}

function cambiaAraña() {

  document.getElementById("scene").src = "3dmodels/araña.glb";
  document.getElementById("menu").style.marginLeft = "-100%";
}

function cambiaCaiman() {

  document.getElementById("scene").src = "3dmodels/caiman.glb";
  document.getElementById("menu").style.marginLeft = "-100%";
}
